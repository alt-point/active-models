...
export type HandlerMapTo<T extends MapSource, RT = any> = (
  source: InstanceType<T>
) => RT;

type MapToMapper<H = HandlerMapTo<MapSource>> = WeakMap<
  MapSource,
  Map<MapTarget, H>
>;
 ...
 * @param Ctor
 */
export const useMapper = <RT = any, T extends MapSource = typeof ActiveModel> (
  Ctor: T
) => {
  ...
  const setMapTo = <RTL = RT> (
    target: MapTarget,
    handler: HandlerMapTo<T, RTL>
  ) => {
    (<MapToMapper<HandlerMapTo<T, RTL>>>MapperTo)
      .get(Ctor)!
      .set(target, handler);
  };
